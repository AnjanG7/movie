...
export const projectsApi = {
  getAll: () => api.get<{ success: boolean; data: { projects: Project[]; total: number; page: number; totalPages: number } }>('/projects'),
  getById: (id: string) => api.get<{ success: boolean; data: Project }>(`/projects/${id}`),
  create: (data: Partial<Project>) => api.post<{ success: boolean; data: Project }>('/projects/create', data),
  assign: (data: { projectId: string; ownerId: string }) => api.put<{ success: boolean; data: Project }>('/projects/assign', data),
};
...
export const budgetApi = {
  getVersions: (projectId: string, query?: { page?: number; limit?: number; type?: string; locked?: boolean }) => {
    const params = new URLSearchParams();
    if (query?.page) params.append('page', query.page.toString());
    if (query?.limit) params.append('limit', query.limit.toString());
    if (query?.type) params.append('type', query.type);
    if (query?.locked !== undefined) params.append('locked', query.locked.toString());
    const queryString = params.toString();
    return api.get<{ success: boolean; data: { versions: BudgetVersion[]; total: number; page: number; totalPages: number } }>(
      `/projects/${projectId}/budget${queryString ? `?${queryString}` : ''}`
    );
  },
  createVersion: (projectId: string, data: { version: string; type: string }) => 
    api.post<{ success: boolean; data: BudgetVersion }>(`/projects/${projectId}/budget`, data),
  addLineItem: (projectId: string, versionId: string, data: Partial<BudgetLineItem>) =>
    api.post<{ success: boolean; data: BudgetLineItem }>(`/projects/${projectId}/budget/${versionId}/lines`, data),
  updateLineItem: (projectId: string, lineId: string, data: Partial<BudgetLineItem>) =>
    api.put<{ success: boolean; data: BudgetLineItem }>(`/projects/${projectId}/budget/lines/${lineId}`, data),
  deleteLineItem: (projectId: string, lineId: string) =>
    api.delete<{ success: boolean; data: { message: string } }>(`/projects/${projectId}/budget/lines/${lineId}`),
  lockBaseline: (projectId: string, versionId: string) =>
    api.post<{ success: boolean; data: BudgetVersion }>(`/projects/${projectId}/budget/${versionId}/lock`),
};
...
export const phasesApi = {
  ...
  getByProject: (projectId: string) => {
    ...
    return projectsApi.getById(projectId).then(res => ({
