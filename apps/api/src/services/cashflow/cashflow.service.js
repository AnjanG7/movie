...
export class CashflowService {
    ...
    async getCashflowForecast(projectId, query = {}) {
    ...
    async upsertCashflowEntry(projectId, data) {
    ...
    async autoComputeCashflow(projectId, weeks = 12) {
    ...
    async getCashflowSummary(projectId) {
        ...
        const currentBalance = forecasts.length > 0 ? forecasts[forecasts.length - 1].cumulative : 0;
    ...
    async recalculateCumulatives(projectId) {
        const forecasts = await prisma.cashflowForecast.findMany({
            where: { projectId },
            orderBy: { weekStart: 'asc' },
        });
