...
export const getPublicityBudgetLines = asyncHandler(async (req, res) => {
  const { projectId } = req.params;
  const result = await publicityService.getPublicityBudgetLines(projectId, req.user);
  res.status(StatusCodes.OK).json(
    new ApiResponse(StatusCodes.OK, result, "Publicity budget lines fetched successfully")
  );
});

export const addPublicityBudgetLine = asyncHandler(async (req, res) => {
  const { projectId } = req.params;
  const line = await publicityService.addPublicityBudgetLine(projectId, req.body, req.user);
  res.status(StatusCodes.CREATED).json(
    new ApiResponse(StatusCodes.CREATED, line, "Publicity budget line added successfully")
  );
});

export const updatePublicityBudgetLine = asyncHandler(async (req, res) => {
  const { lineId } = req.params;
  const updated = await publicityService.updatePublicityBudgetLine(lineId, req.body, req.user);
  res.status(StatusCodes.OK).json(
    new ApiResponse(StatusCodes.OK, updated, "Publicity budget line updated successfully")
  );
});

export const deletePublicityBudgetLine = asyncHandler(async (req, res) => {
  const { lineId } = req.params;
  const result = await publicityService.deletePublicityBudgetLine(lineId, req.user);
  res.status(StatusCodes.OK).json(
    new ApiResponse(StatusCodes.OK, result, "Publicity budget line deleted successfully")
  );
});
