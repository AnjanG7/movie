...
export const createDrawdown = asyncHandler(async (req, res) => {
    ...
    res
        ...
        .json(
            new ApiResponse(StatusCodes.CREATED, drawdown, 'Drawdown created successfully')
        );
});

export const getDrawdowns = asyncHandler(async (req, res) => {
    const { projectId } = req.params;
    const drawdowns = await drawdownService.getDrawdowns(projectId, req.query);
    res
        .status(StatusCodes.OK)
        .json(
            new ApiResponse(StatusCodes.OK, drawdowns, 'Drawdowns fetched successfully')
        );
});
